...
abstract class SimpleOrm
{
    ...
    public static function getTableName ()
    {
        ...
        // assumed config
        return DB_PREFIX . strtolower($className);
    }
    ...
    {
        if ($this->isNew())
            return false;//throw new \Exception('Unable to update object, record is new.');
        ...
        if (!$stmt)
            return false;//throw new \Exception(self::getConnection()->error."\n\n".$sql);

        call_user_func_array(array($stmt, 'bind_param'), array_merge(array(implode($types)), $values));
        $stmt->execute();

        if ($stmt->error)
            return false;//throw new \Exception($stmt->error."\n\n".$sql);
     ...
     * @return mixed
     */
	public static function selectOne($fields = [], $cond = '')
	{
		...
		$sql = "SELECT {$fieldsString} FROM :table {$cond};";
		
		return self::sql($sql, SimpleOrm::FETCH_ONE);
	}
	
	/**
     * Select specific fields by all.
     * 
     * @access public
     * @param array $fields
     * @param string $cond
     * @return mixed
     */
	public static function selectAll($fields = [], $cond = '')
	{
		$fieldsString = empty($fields) ? '*' : implode(',', $fields);
		...
		return self::sql($sql, SimpleOrm::FETCH_MANY);
	}
	
	/**
     * Insert list bunch of records to DB.
     * 
     * @access public
     * @param array $fields
     * @param array $data
	...
	public static function batchInsert($fields, $data)
	{
		...
		foreach ($data as $row) {
			...
			$insertingValues[] = "('{$rowString}')";
		}
		$insertingValuesString = implode(',', $insertingValues);
		
		$sql .= $insertingValuesString;
		
		self::sql($sql, SimpleOrm::FETCH_NONE);
